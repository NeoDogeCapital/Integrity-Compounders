# Financial-model row template — REAL-ESTATE

Authoritative, fixed row structure pulled from `standardized_metrics_list` (templateType: `real-estate`). LOAD this skeleton and FILL values from `company_financials_standardized` keyed by metric id — do NOT re-derive the structure at runtime. Rows are in canonical order; **[T]** marks a subtotal with its build-up (operands use ids with the statement prefix stripped). If a company reports an id not listed here, append it under the nearest group and flag it; if a listed id is absent for a company, render the row as n/a.

## Income Statement (29 rows)
1. `income_statement_property_revenue` — Property Revenue
2. `income_statement_service_and_other_revenue` — Service and Other Revenue
3. `income_statement_total_revenues` **[T]** — Total Revenues  _= +property_revenue +service_and_other_revenue_
4. `income_statement_property_expenses` — Property Expenses
5. `income_statement_service_and_other_expenses` — Service and Other Expenses
6. `income_statement_total_property_expenses` **[T]** — Total Property Expenses  _= +property_expenses +service_and_other_expenses_
7. `income_statement_property_taxes` — Property Taxes
8. `income_statement_gross_profit` **[T]** — Gross Profit  _= +total_revenues −total_property_expenses −property_taxes_
9. `income_statement_selling_general_and_administrative_expenses` — Selling, General & Administrative Expenses
10. `income_statement_depreciation_and_amortization_expenses` — Depreciation & Amortization Expenses
11. `income_statement_other_operating_expenses` — Other Operating Expenses
12. `income_statement_operating_profit` **[T]** — Operating Profit  _= +gross_profit −selling_general_and_administrative_expenses −depreciation_and_amortization_expenses −other_operating_expenses_
13. `income_statement_net_gains_or_losses_on_disposal_of_properties` — Net Gains on Disposal of Properties
14. `income_statement_interest_income` — Interest and Investment Income
15. `income_statement_interest_expense` — Interest Expense
16. `income_statement_non_operating_income` — Non-Operating Income
17. `income_statement_non_operating_income_or_expense` **[T]** — Total Non-Operating Income  _= +interest_income +interest_expense +non_operating_income +net_gains_or_losses_on_disposal_of_properties_
18. `income_statement_income_before_provision_for_income_taxes` **[T]** — Income Before Provision for Income Taxes  _= +operating_profit +non_operating_income_or_expense_
19. `income_statement_provision_for_income_taxes` — Provision for Income Taxes
20. `income_statement_consolidated_net_income` **[T]** — Consolidated Net Income  _= +income_before_provision_for_income_taxes −provision_for_income_taxes_
21. `income_statement_net_income_attributable_to_minority_interests_and_other` — Net Income Attributable to Minority Interests and Other
22. `income_statement_net_income_attributable_to_preferred_dividends` — Net Income Attributable to Preferred Dividends
23. `income_statement_net_income_attributable_to_discontinued_operations` — Net Income Attributable to Discontinued Operations
24. `income_statement_net_income_attributable_to_common_shareholders` **[T]** — Net Income Attributable to Common Shareholders  _= +consolidated_net_income −net_income_attributable_to_minority_interests_and_other −net_income_attributable_to_preferred_dividends +net_income_attributable_to_discontinued_operations_
25. `income_statement_basic_eps` — Basic EPS
26. `income_statement_diluted_eps` — Diluted EPS
27. `income_statement_basic_weighted_average_shares_outstanding` — Basic Weighted Average Shares Outstanding
28. `income_statement_diluted_weighted_average_shares_outstanding` — Diluted Weighted Average Shares Outstanding
29. `shares_outstanding` — Shares Outstanding  _(group: Shares)_

## Balance Sheet (39 rows)
...
7. `balance_sheet_restricted_cash` — Restricted Cash  _(group: Assets)_
8. `balance_sheet_inventories` — Inventories  _(group: Assets)_
9. `balance_sheet_other_current_assets` — Other Current Assets  _(group: Assets)_
10. `balance_sheet_total_current_assets` **[T]** — Total Current Assets  _(group: Assets)_  _= +total_cash_and_cash_equivalents +total_trade_receivables +restricted_cash +inventories +other_current_assets_
...
19. `balance_sheet_short_term_debt` — Short-Term Debt  _(group: Liabilities)_
20. `balance_sheet_current_portion_of_long_term_debt` — Current Portion of Long-Term Debt  _(group: Liabilities)_
21. `balance_sheet_current_portion_of_leases` — Current Portion of Leases  _(group: Liabilities)_
22. `balance_sheet_unearned_revenue` — Unearned Revenue  _(group: Liabilities)_
23. `balance_sheet_other_current_liabilities` — Other Current Liabilities  _(group: Liabilities)_
24. `balance_sheet_total_current_liabilities` **[T]** — Total Current Liabilities  _(group: Liabilities)_  _= +accounts_payable +accrued_expenses +short_term_debt +current_portion_of_long_term_debt +current_portion_of_leases +unearned_revenue +other_current_liabilities_
25. `balance_sheet_long_term_debt` — Long-Term Debt  _(group: Liabilities)_
26. `balance_sheet_leases` — Leases  _(group: Liabilities)_
27. `balance_sheet_other_long_term_liabilities` — Other Long-Term Liabilities  _(group: Liabilities)_
28. `balance_sheet_total_long_term_liabilities` **[T]** — Total Long-Term Liabilities  _(group: Liabilities)_  _= +long_term_debt +leases +other_long_term_liabilities_
29. `balance_sheet_total_liabilities` **[T]** — Total Liabilities  _(group: Liabilities)_  _= +total_current_liabilities +total_long_term_liabilities_
30. `balance_sheet_preferred_stock` — Preferred Stock  _(group: Equity)_
31. `balance_sheet_common_stock` — Common Stock  _(group: Equity)_
32. `balance_sheet_treasury_stock` — Treasury Stock  _(group: Equity)_
33. `balance_sheet_additional_paid_in_capital` — Additional Paid-in Capital  _(group: Equity)_
34. `balance_sheet_accumulated_other_comprehensive_income` — Accumulated Other Comprehensive Income  _(group: Equity)_
35. `balance_sheet_retained_earnings` — Retained Earnings  _(group: Equity)_
36. `balance_sheet_total_common_shareholders_equity` **[T]** — Total Common Shareholders' Equity  _(group: Equity)_  _= +preferred_stock +common_stock +treasury_stock +additional_paid_in_capital +accumulated_other_comprehensive_income +retained_earnings_
37. `balance_sheet_minority_interests_and_other` — Minority Interests and Other  _(group: Equity)_
38. `balance_sheet_total_shareholders_equity` **[T]** — Total Shareholders' Equity  _(group: Equity)_  _= +total_common_shareholders_equity +minority_interests_and_other_
39. `balance_sheet_total_liabilities_and_shareholders_equity` **[T]** — Total Liabilities and Shareholders' Equity  _(group: Equity)_  _= +total_liabilities +total_shareholders_equity_
