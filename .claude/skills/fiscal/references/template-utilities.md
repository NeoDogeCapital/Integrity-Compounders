# Financial-model row template — UTILITIES

Authoritative, fixed row structure pulled from `standardized_metrics_list` (templateType: `utilities`). LOAD this skeleton and FILL values from `company_financials_standardized` keyed by metric id — do NOT re-derive the structure at runtime. Rows are in canonical order; **[T]** marks a subtotal with its build-up (operands use ids with the statement prefix stripped). If a company reports an id not listed here, append it under the nearest group and flag it; if a listed id is absent for a company, render the row as n/a.

## Income Statement (25 rows)
1. `income_statement_total_revenues` — Total Revenues
2. `fuel_and_purchased_power_expense` — Fuel and Purchased Power Expense
3. `income_statement_operations_and_maintenance_expenses` — Operations and Maintenance Expenses
4. `income_statement_gross_profit` **[T]** — Gross Profit  _= +total_revenues −fuel_and_purchased_power_expense −operations_and_maintenance_expenses_
5. `income_statement_depreciation_and_amortization_expenses` — Depreciation & Amortization Expenses
6. `income_statement_taxes_other_than_income_tax` — Taxes Other than Income Tax
7. `income_statement_net_gains_or_losses_on_disposal_of_operating_assets` — Net Gains on Disposal of Operating Assets
8. `income_statement_other_operating_expenses` — Other Operating Expenses
9. `income_statement_operating_profit` **[T]** — Operating Profit  _= +gross_profit −depreciation_and_amortization_expenses −taxes_other_than_income_tax +net_gains_or_losses_on_disposal_of_operating_assets −other_operating_expenses_
...
8. `balance_sheet_short_term_regulatory_assets` — Short-Term Regulatory Assets  _(group: Assets)_
9. `balance_sheet_other_current_assets` — Other Current Assets  _(group: Assets)_
10. `balance_sheet_total_current_assets` **[T]** — Total Current Assets  _(group: Assets)_  _= +total_cash_and_cash_equivalents +total_trade_receivables +inventories +short_term_regulatory_assets +other_current_assets_
...
12. `balance_sheet_special_use_funds` — Special Use Funds  _(group: Assets)_
13. `balance_sheet_net_intangible_assets` — Net Intangible Assets  _(group: Assets)_
14. `balance_sheet_goodwill` — Goodwill  _(group: Assets)_
15. `balance_sheet_long_term_investments` — Long-Term Investments  _(group: Assets)_
16. `balance_sheet_long_term_regulatory_assets` — Long-Term Regulatory Assets  _(group: Assets)_
17. `balance_sheet_other_long_term_assets` — Other Long-Term Assets  _(group: Assets)_
18. `balance_sheet_total_assets` **[T]** — Total Assets  _(group: Assets)_  _= +total_current_assets +net_property_plant_and_equipment +special_use_funds +net_intangible_assets +goodwill +long_term_investments +long_term_regulatory_assets +other_long_term_assets_
19. `balance_sheet_accounts_payable` — Accounts Payable  _(group: Liabilities)_
20. `balance_sheet_accrued_expenses` — Accrued Expenses  _(group: Liabilities)_
...
25. `balance_sheet_short_term_regulatory_liabilities` — Short-Term Regulatory Liabilities  _(group: Liabilities)_
26. `balance_sheet_other_current_liabilities` — Other Current Liabilities  _(group: Liabilities)_
27. `balance_sheet_total_current_liabilities` **[T]** — Total Current Liabilities  _(group: Liabilities)_  _= +accounts_payable +accrued_expenses +short_term_debt +current_portion_of_long_term_debt +current_portion_of_leases +unearned_revenue +short_term_regulatory_liabilities +other_current_liabilities_
28. `balance_sheet_long_term_debt` — Long-Term Debt  _(group: Liabilities)_
29. `balance_sheet_leases` — Leases  _(group: Liabilities)_
30. `balance_sheet_long_term_regulatory_liabilities` — Long-Term Regulatory Liabilities  _(group: Liabilities)_
31. `balance_sheet_other_long_term_liabilities` — Other Long-Term Liabilities  _(group: Liabilities)_
32. `balance_sheet_total_long_term_liabilities` **[T]** — Total Long-Term Liabilities  _(group: Liabilities)_  _= +long_term_debt +leases +long_term_regulatory_liabilities +other_long_term_liabilities_
33. `balance_sheet_total_liabilities` **[T]** — Total Liabilities  _(group: Liabilities)_  _= +total_current_liabilities +total_long_term_liabilities_
34. `balance_sheet_preferred_stock` — Preferred Stock  _(group: Equity)_
35. `balance_sheet_common_stock` — Common Stock  _(group: Equity)_
36. `balance_sheet_treasury_stock` — Treasury Stock  _(group: Equity)_
37. `balance_sheet_additional_paid_in_capital` — Additional Paid-in Capital  _(group: Equity)_
38. `balance_sheet_accumulated_other_comprehensive_income` — Accumulated Other Comprehensive Income  _(group: Equity)_
39. `balance_sheet_retained_earnings` — Retained Earnings  _(group: Equity)_
40. `balance_sheet_total_common_shareholders_equity` **[T]** — Total Common Shareholders' Equity  _(group: Equity)_  _= +preferred_stock +common_stock +treasury_stock +additional_paid_in_capital +accumulated_other_comprehensive_income +retained_earnings_
41. `balance_sheet_minority_interests_and_other` — Minority Interests and Other  _(group: Equity)_
42. `balance_sheet_total_shareholders_equity` **[T]** — Total Shareholders' Equity  _(group: Equity)_  _= +total_common_shareholders_equity +minority_interests_and_other_
43. `balance_sheet_total_liabilities_and_shareholders_equity` **[T]** — Total Liabilities and Shareholders' Equity  _(group: Equity)_  _= +total_liabilities +total_shareholders_equity_
